...
class Work:
    ...
    def work(self, bohater, type=0, adventure=Adventure(self.dif_level)):
        ...
        if not adventure.in_action:
            ...
            if bohater.passive[type].actual - self.cost < 0:
                print("Nie posiadasz wystarczająco staminy")
            else:
                time.sleep(random.randrange(1,10,1)) #
                bohater.passive[0].actual -= self.cost
                bohater.riches+=self.reward
        else: print("Aby skorzystać zakończ misje...")
...
class Act:
    ...
    def act(self, bohater, adventure=Adventure(self.dif_level)):
        #jeśli przygoda nie zostanie użyta w wywołaniu metody to zostaje utworzona nowa z in_action = False
        if adventure.in_action:
            if bohater.passive[0].actual - self.cost < 0:
                print("Nie posiadasz wystarczająco staminy")
            else:
                bohater.passive[0].actual -= self.cost
                bohater.riches+=self.reward
        else: print("Aby skorzystać rozpocznij misje...")
...
class Rest:
    ...
    def regeneration(self, bohater, adventure=Adventure(self.dif_level)):
        #jeśli przygoda nie zostanie użyta w wywołaniu metody to zostaje utworzona nowa z in_action = False
        if not adventure.in_action:
            while bohater.passive[0].actual != bohater.passive[0].max:
                time.sleep(1) #póki co usypia na sekundę
                if bohater.passive[0].actual + self.stamina>bohater.passive[0].max:
                    bohater.passive[0].actual = bohater.passive[0].max
                else: bohater.passive[0].actual += self.stamina
        else: 
